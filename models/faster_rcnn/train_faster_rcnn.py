...
#
# Faster R-CNN training script with custom training loop.
# Custom training loop ile Faster R-CNN training script.
#
...
def load_resume_state(
    ...
    #
    # Returns / Donus:
    #     dict with "start_epoch" and "checkpoint_cfg" for caller decisions.
    #     "start_epoch" ve "checkpoint_cfg" iceren dict, cagrici karari icin.
    print(f"[Faster-RCNN] Resuming from {resume_path}")
    ...
    print(f"[Faster-RCNN] Resumed at epoch {start_epoch} "
...
def train(data_dir: str, cfg: Optional[dict] = None) -> None:
    ...
    if c["DEVICE"].startswith("cuda") and not torch.cuda.is_available():
        print(f"[Faster-RCNN] CUDA not available, falling back to CPU.")
        c["DEVICE"] = "cpu"

    is_resume = c.get("RESUME_FROM") is not None

    print(f"[Faster-RCNN] Backbone:  {c['BACKBONE']}")
    print(f"[Faster-RCNN] Device:    {c['DEVICE']}")
    print(f"[Faster-RCNN] Batch:     {c['BATCH_SIZE']}, Epochs: {c['EPOCHS']}")
    print(f"[Faster-RCNN] LR0:       {c['LR0']}, LRF: {c['LRF']}")
    print(f"[Faster-RCNN] Patience:  {c['PATIENCE']}")
    if is_resume:
        print(f"[Faster-RCNN] Resume:    {c['RESUME_FROM']}")

    train_loader, val_loader, class_names, num_classes = build_dataloaders(data_dir, c)
    print(f"[Faster-RCNN] Train: {len(train_loader.dataset)} images, "
          f"{len(train_loader)} batches")
    print(f"[Faster-RCNN] Val:   {len(val_loader.dataset)} images, "
          f"{len(val_loader)} batches")
    print(f"[Faster-RCNN] Classes: {class_names} (num_classes={num_classes} incl. bg)")
    ...
    print(f"[Faster-RCNN] Total params: {params['total_millions']:.2f}M")
    ...
    if is_resume:
        ...
        print(f"[Faster-RCNN] Resume dir: {save_dir}")

        # Load model and optimizer state.
        # Model ve optimizer state'ini yukle.
        resume_state = load_resume_state(c["RESUME_FROM"], model, optimizer, c["DEVICE"])
        start_epoch = resume_state["start_epoch"]

        # Load best.pt info to restore best_map50 and patience counter.
        # best_map50 ve patience counter'i geri yuklemek icin best.pt bilgisini yukle.
        best_state = load_best_state(save_dir / "best.pt")
        best_map50 = best_state["best_map50"]
        best_epoch = best_state["best_epoch"]
        # Patience counter: how many epochs since best_epoch.
        # Patience counter: best_epoch'tan beri gecen epoch sayisi.
        patience_counter = max(0, (start_epoch - 1) - best_epoch)
        print(f"[Faster-RCNN] Best so far: mAP@0.5={best_map50:.4f} @ epoch {best_epoch}")
        print(f"[Faster-RCNN] Patience counter: {patience_counter}/{c['PATIENCE']}")
    else:
        save_dir = resolve_run_dir(c["PROJECT"], c["NAME"])
        save_dir.mkdir(parents=True, exist_ok=True)
        excel_path = save_dir / c["OUTPUT_XLSX"]
        print(f"[Faster-RCNN] Output dir: {save_dir}")
        build_excel(excel_path, class_names)

    val_gt_path = Path(data_dir) / "val" / "_annotations.coco.json"
    coco_gt = COCO(str(val_gt_path))

    print(f"\n[Faster-RCNN] Starting training from epoch {start_epoch}...\n")
    ...
    for epoch in range(start_epoch, c["EPOCHS"]):
        ...
        if patience_counter >= c["PATIENCE"]:
            print(f"[Faster-RCNN] Early stopping at epoch {epoch}. "
                  f"Best: epoch {best_epoch}, mAP@0.5={best_map50:.4f}")
            break

    print(f"\n[Faster-RCNN] Training finished.")
