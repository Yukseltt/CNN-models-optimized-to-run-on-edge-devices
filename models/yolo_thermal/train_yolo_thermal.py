# train_yolo_thermal.py
#
# YOLO-Thermal training: YOLOv8-nano + thermal-aware augmentation + Excel logging.
# YOLO-Thermal egitimi: YOLOv8-nano + termal-aware augmentation + Excel kaydi.
#
# What makes this "thermal"-aware (vs the plain yolov8 baseline):
#     - HSV augmentation is OFF (thermal has no color information worth
#       perturbing; HSV shifts would only add noise).
#     - Geometric augmentations (mosaic, flip, scale) are kept because they
#       remain valid in thermal.
# Termal-aware kismi nedir (yolov8 baseline'dan farki):
#     - HSV augmentation KAPALI (termal goruntude renk bilgisi yok; HSV
#       shift'leri sadece gurultu eklerdi).
#     - Geometrik augmentation'lar (mosaic, flip, scale) korunuyor; termalde
#       de gecerli.
#
# Mirrors the layout of models/yolov11/train_yolo11n_fcl.py:
# models/yolov11/train_yolo11n_fcl.py duzenini taklit eder:
#     - NANO_CFG dict
#     - read_class_names / resolve_excel_path helpers
#     - Excel logger (build_excel / append_epoch_row)
#     - Callback class with on_pretrain_routine_end / on_fit_epoch_end /
#       on_train_end (no loss injection here)
#     - train() entry point + __main__ block
...
# Training configuration with thermal-aware augmentation.
# Termal-aware augmentation'la egitim konfigurasyonu.
NANO_CFG = {
    "IMG_SIZE":       640,
    "DEVICE":         0,
    "EPOCHS":         100,
    ...
    # Thermal-specific: kill all colour-space augmentation.
    # Termal-ozel: tum renk-uzayi augmentation'lari kapali.
    "HSV_H":          0.0,
    "HSV_S":          0.0,
    "HSV_V":          0.0,

    # Geometric augmentation still useful on thermal.
    # Geometrik augmentation termalde de yararli.
    "MOSAIC":         1.0,
    "MIXUP":          0.0,
    "FLIPUD":         0.0,
    "FLIPLR":         0.5,
...
# Callbacks for Excel logging (no loss injection — uses default v8 loss).
# Excel kaydi icin callback'ler (loss injection yok — varsayilan v8 loss).
class ThermalTrainingCallbacks:
    ...
    def on_pretrain_routine_end(self, trainer) -> None:
        ...
        LOGGER.info(f"[YOLO-Thermal] Excel will be written to: {self.output_path}")
    ...
    def on_train_end(self, trainer) -> None:
        LOGGER.info(
            f"[YOLO-Thermal] Training finished. Output: {self.output_path}"
        )
...
def train(
    ...
    LOGGER.info(f"[YOLO-Thermal] Classes: {class_names}, seed={c['SEED']}")
    LOGGER.info(f"[YOLO-Thermal] Runs directory: {c['PROJECT']}")
    if is_resume:
        LOGGER.info(f"[YOLO-Thermal] RESUME from: {resume_from}")

    # Base weights: YOLOv8n (auto-downloaded by Ultralytics on first run).
    # Base agirliklar: YOLOv8n (Ultralytics ilk kosuda otomatik indirir).
    model = YOLO(resume_from if is_resume else "yolov8n.pt")

    cbs = ThermalTrainingCallbacks(
