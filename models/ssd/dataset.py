# dataset.py
#
# Custom COCO Dataset for Torchvision SSD.
# Torchvision SSD icin custom COCO Dataset.
#
...
#
# Returns (image_tensor, target_dict) per Torchvision's SSD contract
# (same contract as Faster R-CNN: list of images + list of target dicts).
# Torchvision SSD beklentisine gore (image_tensor, target_dict) dondurur
# (Faster R-CNN ile ayni: goruntu listesi + target dict listesi).
#
# SSD has an internal GeneralizedRCNNTransform-like resizer/normalizer,
# so we feed raw images at original resolution in [0, 1] range; the model
# rescales internally to 300x300 (SSD300) or 320x320 (SSDLite).
# SSD'nin kendi icinde GeneralizedRCNNTransform benzeri resize/normalize
# var, bu yuzden orijinal cozunurlukte [0, 1] araliginda raw goruntuler
# veriyoruz; model dahili olarak 300x300 (SSD300) veya 320x320 (SSDLite)
...
class CocoDetectionDataset(Dataset):
    # Custom COCO dataset that returns SSD compatible targets.
    # SSD uyumlu target dondurur custom COCO dataset.
    #
    # Args / Parametreler:
    ...
    def __init__(
        ...
        # Torchvision SSD, 0 etiketini background icin ayirir.
