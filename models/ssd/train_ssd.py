# train_ssd.py
#
# SSD (SSDLite320 + MobileNetV3-Large) training script with custom training loop.
# Custom training loop ile SSD (SSDLite320 + MobileNetV3-Large) training script.
#
...
#     - last.pt'den resume
#
# SSD-specific notes / SSD'ye ozel notlar:
#     - Loss dict has two components: "bbox_regression" and "classification".
#     - Loss dict iki bilesenli: "bbox_regression" ve "classification".
#     - SSDLite uses BatchNorm in detection head; BN-eval trick still applies.
#     - SSDLite detection head'inde BatchNorm var; BN-eval trick yine gecerli.
#
# Usage / Kullanim:
#     from train_ssd import train
#     train(data_dir="/path/to/coco_root", cfg={"BACKBONE": "mobilenet", ...})
#
...
from model_factory import create_ssd, count_parameters
...
DEFAULT_CFG = {
    ...
    "NAME":           "ssdlite_mobilenet_default",
...
def load_resume_state(
    ...
    print(f"[SSD] Resuming from {resume_path}")
    ...
    print(f"[SSD] Resumed at epoch {start_epoch} "
...
def train(data_dir: str, cfg: Optional[dict] = None) -> None:
    ...
    if c["DEVICE"].startswith("cuda") and not torch.cuda.is_available():
        print(f"[SSD] CUDA not available, falling back to CPU.")
        c["DEVICE"] = "cpu"

    is_resume = c.get("RESUME_FROM") is not None

    # cuDNN autotune: SSDLite's internal transform reshapes inputs to a fixed
    # 320x320, so shapes are constant -> autotune is a free win.
    # cuDNN autotune: SSDLite'in dahili transform'u girdiyi sabit 320x320'ye
    ...
    print(f"[SSD] Backbone:  {c['BACKBONE']} (SSDLite320 + MobileNetV3-Large)")
    print(f"[SSD] Device:    {c['DEVICE']}")
    print(f"[SSD] Batch:     {c['BATCH_SIZE']}, Epochs: {c['EPOCHS']}, Workers: {c['WORKERS']}")
    print(f"[SSD] LR0:       {c['LR0']}, LRF: {c['LRF']}")
    print(f"[SSD] Patience:  {c['PATIENCE']}")
    print(f"[SSD] AMP:       {use_amp} (dtype={amp_dtype if use_amp else 'fp32'})  cuDNN benchmark: {torch.backends.cudnn.benchmark}")
    try:
        _soft, _hard = resource.getrlimit(resource.RLIMIT_NOFILE)
        print(f"[SSD] MP share:  {mp.get_sharing_strategy()}  RLIMIT_NOFILE: soft={_soft}, hard={_hard}")
    except (ValueError, OSError):
        pass
    if is_resume:
        print(f"[SSD] Resume:    {c['RESUME_FROM']}")

    train_loader, val_loader, class_names, num_classes = build_dataloaders(data_dir, c)
    print(f"[SSD] Train: {len(train_loader.dataset)} images, "
          f"{len(train_loader)} batches")
    print(f"[SSD] Val:   {len(val_loader.dataset)} images, "
          f"{len(val_loader)} batches")
    print(f"[SSD] Classes: {class_names} (num_classes={num_classes} incl. bg)")
    ...
    print(f"[SSD] Total params: {params['total_millions']:.2f}M")
    ...
    if is_resume:
        ...
        print(f"[SSD] Resume dir: {save_dir}")
        ...
        print(f"[SSD] Best so far: mAP@0.5={best_map50:.4f} @ epoch {best_epoch}")
        print(f"[SSD] Patience counter: {patience_counter}/{c['PATIENCE']}")
    else:
        save_dir = resolve_run_dir(c["PROJECT"], c["NAME"])
        save_dir.mkdir(parents=True, exist_ok=True)
        excel_path = save_dir / c["OUTPUT_XLSX"]
        print(f"[SSD] Output dir: {save_dir}")
        build_excel(excel_path, class_names)

    val_gt_path = Path(data_dir) / "val" / "_annotations.coco.json"
    coco_gt = COCO(str(val_gt_path))

    print(f"\n[SSD] Starting training from epoch {start_epoch}...\n")
