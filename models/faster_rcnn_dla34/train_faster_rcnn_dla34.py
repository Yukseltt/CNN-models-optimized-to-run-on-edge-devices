# train_faster_rcnn_dla34.py
#
# Training script for Faster R-CNN with DLA-34 + FPN backbone.
# DLA-34 + FPN backbone'lu Faster R-CNN icin egitim scripti.
#
# Same training loop as faster_rcnn/train_faster_rcnn.py: manual warmup +
# cosine LR, hybrid validation (BN-eval-mode val loss + full-eval predictions),
# pycocotools mAP, custom YOLO-style precision/recall/F1, Excel logging,
# best/last checkpointing, early stopping, resume from last.pt.
#
# faster_rcnn/train_faster_rcnn.py ile ayni egitim dongusu: manuel warmup +
# cosine LR, hybrid validation (BN-eval mode val loss + tam eval tahminler),
# pycocotools mAP, custom YOLO tarzi precision/recall/F1, Excel logging,
# best/last checkpoint, early stopping, last.pt'den resume.
#
# Shared utilities (dataset, eval metrics, Excel logger) are reused from the
# sibling faster_rcnn/ folder to avoid duplication.
# Ortak yardimcilar (dataset, eval metrics, Excel logger) duplikasyon olmasin
# diye komsu faster_rcnn/ klasorunden tekrar kullanilir.
...
# Reuse dataset, metrics, logger from sibling faster_rcnn/ module.
# Dataset, metrics, logger'i komsu faster_rcnn/ modulunden tekrar kullan.
SIBLING_DIR = Path(__file__).resolve().parent.parent / "faster_rcnn"
...
# Local backbone factory.
# Lokal backbone factory.
sys.path.insert(0, str(Path(__file__).resolve().parent))
from model_factory import create_faster_rcnn_dla34, count_parameters
...
#
# DLA-34 backbone is only ImageNet-pretrained (no COCO detector weights), so
# we use a longer warmup and a slightly smaller LR than the resnet50 baseline.
# DLA-34 backbone sadece ImageNet-pretrained (COCO detector agirligi yok),
# bu yuzden resnet50 baseline'a kiyasla daha uzun warmup ve biraz kucuk LR.
...
DEFAULT_CFG = {
    ...
    "NAME":           "faster_rcnn_dla34_default",
...
def train(data_dir: str, cfg: Optional[dict] = None) -> None:
    ...
    if c["DEVICE"].startswith("cuda") and not torch.cuda.is_available():
        print(f"[FRCNN-DLA34] CUDA not available, falling back to CPU.")
        c["DEVICE"] = "cpu"

    is_resume = c.get("RESUME_FROM") is not None

    print(f"[FRCNN-DLA34] Backbone:  dla34 + FPN")
    ...
    if is_resume:
        print(f"[FRCNN-DLA34] Resume:    {c['RESUME_FROM']}")

    train_loader, val_loader, class_names, num_classes = build_dataloaders(data_dir, c)
    print(f"[FRCNN-DLA34] Train: {len(train_loader.dataset)} images, "
          f"{len(train_loader)} batches")
    print(f"[FRCNN-DLA34] Val:   {len(val_loader.dataset)} images, "
          f"{len(val_loader)} batches")
    print(f"[FRCNN-DLA34] Classes: {class_names} (num_classes={num_classes} incl. bg)")

    model = create_faster_rcnn_dla34(
    ...
    if is_resume:
        ...
        print(f"[FRCNN-DLA34] Patience counter: {patience_counter}/{c['PATIENCE']}")
    else:
