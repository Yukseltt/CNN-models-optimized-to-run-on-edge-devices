# dataset.py
#
# Custom COCO Dataset for Torchvision RetinaNet (MLPD-adapted).
# Torchvision RetinaNet (MLPD-adapted) icin custom COCO Dataset.
#
...
#
# Returns (image_tensor, target_dict) per Torchvision's MLPD contract
# (same contract as RetinaNet: list of images + list of target dicts).
# Torchvision RetinaNet (MLPD-adapted) beklentisine gore (image_tensor, target_dict) dondurur
# (RetinaNet ile ayni: goruntu listesi + target dict listesi).
#
# MLPD has an internal GeneralizedRCNNTransform-like resizer/normalizer,
# so we feed raw images at original resolution in [0, 1] range; the model
# rescales internally to 300x300 (MLPD300) or 320x320 (MLPDLite).
# MLPD'nin kendi icinde GeneralizedRCNNTransform benzeri resize/normalize
# var, bu yuzden orijinal cozunurlukte [0, 1] araliginda raw goruntuler
# veriyoruz; model dahili olarak 300x300 (MLPD300) veya 320x320 (MLPDLite)
...
class CocoDetectionDataset(Dataset):
    # Custom COCO dataset that returns MLPD compatible targets.
    # MLPD uyumlu target dondurur custom COCO dataset.
    #
    # Args / Parametreler:
    ...
    def __init__(
        ...
        #
        # Torchvision RetinaNet (MLPD-adapted) reserves label 0 for background.
        # Torchvision RetinaNet (MLPD-adapted), 0 etiketini background icin ayirir.
