# train_mlpd.py
#
# MLPD-flavored RetinaNet (ResNet50 + FPN) training script with custom training loop.
# Custom training loop ile MLPD-flavored RetinaNet (ResNet50 + FPN) training script.
#
...
#     - last.pt'den resume
#
# MLPD-specific notes / MLPD'ya ozel notlar:
#     - Loss dict has two components: "bbox_regression" and "classification".
#     - Loss dict iki bilesenli: "bbox_regression" ve "classification".
#     - RetinaNet uses BatchNorm in detection head; BN-eval trick still applies.
#     - RetinaNet detection head'inde BatchNorm var; BN-eval trick yine gecerli.
#
# Usage / Kullanim:
...
from model_factory import create_mlpd, count_parameters
...
SSD_LOSS_KEYS = ("bbox_regression", "classification")
...
def _empty_loss_dict() -> dict:
    return {k: 0.0 for k in SSD_LOSS_KEYS}
...
def load_resume_state(
    ...
    print(f"[MLPD] Resuming from {resume_path}")
    ...
    print(f"[MLPD] Resumed at epoch {start_epoch} "
...
def train(data_dir: str, cfg: Optional[dict] = None) -> None:
    ...
    if c["DEVICE"].startswith("cuda") and not torch.cuda.is_available():
        print(f"[MLPD] CUDA not available, falling back to CPU.")
        c["DEVICE"] = "cpu"

    is_resume = c.get("RESUME_FROM") is not None

    # cuDNN autotune: RetinaNet's internal transform reshapes inputs to a fixed
    # 320x320, so shapes are constant -> autotune is a free win.
    # cuDNN autotune: RetinaNet'in dahili transform'u girdiyi sabit 320x320'ye
    ...
    print(f"[MLPD] Backbone:  {c['BACKBONE']} (RetinaNet (ResNet50 + FPN))")
    print(f"[MLPD] Device:    {c['DEVICE']}")
    print(f"[MLPD] Batch:     {c['BATCH_SIZE']}, Epochs: {c['EPOCHS']}, Workers: {c['WORKERS']}")
    print(f"[MLPD] LR0:       {c['LR0']}, LRF: {c['LRF']}")
    print(f"[MLPD] Patience:  {c['PATIENCE']}")
    print(f"[MLPD] AMP:       {use_amp} (dtype={amp_dtype if use_amp else 'fp32'})  cuDNN benchmark: {torch.backends.cudnn.benchmark}")
    try:
        _soft, _hard = resource.getrlimit(resource.RLIMIT_NOFILE)
        print(f"[MLPD] MP share:  {mp.get_sharing_strategy()}  RLIMIT_NOFILE: soft={_soft}, hard={_hard}")
    except (ValueError, OSError):
        pass
    if is_resume:
        print(f"[MLPD] Resume:    {c['RESUME_FROM']}")

    train_loader, val_loader, class_names, num_classes = build_dataloaders(data_dir, c)
    print(f"[MLPD] Train: {len(train_loader.dataset)} images, "
          f"{len(train_loader)} batches")
    print(f"[MLPD] Val:   {len(val_loader.dataset)} images, "
          f"{len(val_loader)} batches")
    print(f"[MLPD] Classes: {class_names} (num_classes={num_classes} incl. bg)")
    ...
    print(f"[MLPD] Total params: {params['total_millions']:.2f}M")
    ...
    if is_resume:
        ...
        print(f"[MLPD] Resume dir: {save_dir}")
        ...
        print(f"[MLPD] Best so far: mAP@0.5={best_map50:.4f} @ epoch {best_epoch}")
        print(f"[MLPD] Patience counter: {patience_counter}/{c['PATIENCE']}")
    else:
        save_dir = resolve_run_dir(c["PROJECT"], c["NAME"])
        save_dir.mkdir(parents=True, exist_ok=True)
        excel_path = save_dir / c["OUTPUT_XLSX"]
        print(f"[MLPD] Output dir: {save_dir}")
        build_excel(excel_path, class_names)

    val_gt_path = Path(data_dir) / "val" / "_annotations.coco.json"
    coco_gt = COCO(str(val_gt_path))

    print(f"\n[MLPD] Starting training from epoch {start_epoch}...\n")
    ...
    for epoch in range(start_epoch, c["EPOCHS"]):
        ...
        if patience_counter >= c["PATIENCE"]:
            print(f"[MLPD] Early stopping at epoch {epoch}. "
                  f"Best: epoch {best_epoch}, mAP@0.5={best_map50:.4f}")
            break

    print(f"\n[MLPD] Training finished.")
