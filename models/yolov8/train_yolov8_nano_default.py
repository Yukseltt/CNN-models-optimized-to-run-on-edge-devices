# train_yolov8_nano_default.py
#
# YOLOv8-nano training with Ultralytics default hyperparameters.
# Ultralytics varsayilan hyperparametreleriyle YOLOv8-nano egitimi.
#
...
class DefaultTrainingCallbacks:
    ...
    def on_fit_epoch_end(self, trainer) -> None:
        ...
        if all(v is not None for v in [box_t, cls_t, dfl_t, map50]):
            LOGGER.info(
                f"[Default-Train] Epoch {epoch:3d} -> "
                f"box={box_t:.4f}  cls={cls_t:.4f}  dfl={dfl_t:.4f}  "
                f"mAP@0.5={map50:.4f}"
            )
        else:
            LOGGER.info(f"[Default-Train] Epoch {epoch} saved.")
    ...
    def on_train_end(self, trainer) -> None:
        LOGGER.info(
            f"[Default-Train] Training finished. Output: {self.output_path}"
        )
...
def train(
    ...
    LOGGER.info(
        f"[Default-Train] Classes: {class_names}, seed={c['SEED']}"
    )
    LOGGER.info(f"[Default-Train] Runs directory: {c['PROJECT']}")
    LOGGER.info(f"[Default-Train] Mosaic={c['MOSAIC']}, FlipLR={c['FLIPLR']}")
