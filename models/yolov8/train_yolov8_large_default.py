# train_yolov8_large_default.py
#
# YOLOv8-large training (teacher for distillation) with Ultralytics default hyperparameters.
# Ultralytics varsayilan hyperparametreleriyle YOLOv8-large egitimi (distillation teacher).
#
# No loss modifications, no augmentation overrides beyond defaults.
# Loss modifikasyonu yok, default disinda augmentation override'i yok.
#
# Mosaic, fliplr, and other defaults are kept as Ultralytics ships them.
# Mosaic, fliplr ve diger varsayilanlar Ultralytics'in geldigi gibi tutulur.
#
# Install / Kurulum:
...
# Tekrarlanabilirlik icin Ultralytics varsayilan degerleri acikca yazildi.
#
# Source / Kaynak:
#     https://docs.ultralytics.com/usage/cfg/
#     Ultralytics 8.4.x defaults at the time of this script.
#     Bu scriptin yazildigi tarihteki Ultralytics 8.4.x varsayilanlari.
...
def build_excel(output_path: Path, class_names: list[str]) -> None:
    ...
    wb.save(output_path)
    LOGGER.info(f"[Teacher-Train] Excel created: {output_path}")
...
# Trainer callbacks for metric logging only.
# Sadece metrik kaydi icin trainer callback'leri.

class DefaultTrainingCallbacks:
    # Callback class for default training.
    # Default egitim icin callback sinifi.
    #
    # No loss modification. Pure logging.
    # Loss modifikasyonu yok. Sadece logging.
    ...
    def on_pretrain_routine_end(self, trainer) -> None:
        ...
        LOGGER.info(f"[Teacher-Train] Excel will be written to: {self.output_path}")
    ...
    def on_fit_epoch_end(self, trainer) -> None:
        ...
        if all(v is not None for v in [box_t, cls_t, dfl_t, map50]):
            LOGGER.info(
                f"[Teacher-Train] Epoch {epoch:3d} -> "
                f"box={box_t:.4f}  cls={cls_t:.4f}  dfl={dfl_t:.4f}  "
                f"mAP@0.5={map50:.4f}"
            )
        else:
            LOGGER.info(f"[Teacher-Train] Epoch {epoch} saved.")
    ...
    def on_train_end(self, trainer) -> None:
        LOGGER.info(
            f"[Teacher-Train] Training finished. Output: {self.output_path}"
        )
...
def train(
    ...
    # Train YOLOv8-nano with Ultralytics default hyperparameters.
    # YOLOv8-nano'yu Ultralytics varsayilan hyperparametreleriyle egit.
    c = {**LARGE_CFG, **(cfg or {})}
    ...
    LOGGER.info(
        f"[Teacher-Train] Classes: {class_names}, seed={c['SEED']}"
    )
    LOGGER.info(f"[Teacher-Train] Runs directory: {c['PROJECT']}")
    LOGGER.info(f"[Teacher-Train] Mosaic={c['MOSAIC']}, FlipLR={c['FLIPLR']}")
    ...
    model.train(
        ...
        warmup_momentum=c["WARMUP_MOMENTUM"],
        warmup_bias_lr=c["WARMUP_BIAS_LR"],
        amp=c["AMP"],
        cos_lr=c["COS_LR"],
