# train_yolo26_nano_cb.py
# YOLO26-nano training with Class-Balanced Loss and Excel metric logging.
...
NANO_CFG = {
    "IMG_SIZE":       640,
    "DEVICE":         0,
    "EPOCHS":         100, # Kullanici istegine gore 100 olarak guncellendi
    "BATCH_SIZE":     64,
    "PATIENCE":       30,
    "WORKERS":        4,
    "CACHE":          False,
...
def inject_cb_weights(model, class_counts: list[int], beta: float, device: torch.device) -> None:
    weights = compute_cb_weights(class_counts=class_counts, beta=beta, normalize=True, device=device)
    model.class_weights = weights.view(1, 1, -1)
    LOGGER.info(f"[CB-Train] class_weights injected - beta={beta}, nc={len(class_counts)}, min={weights.min():.4f}, max={weights.max():.4f}")
...
class CBTrainingCallbacks:
    def __init__(self, output_path: Path, class_counts: list[int], class_names: list[str], beta: float):
        self.output_path, self.class_counts, self.class_names, self.beta = output_path, class_counts, class_names, beta

    def on_pretrain_routine_end(self, trainer) -> None:
        raw = trainer.model
        while isinstance(raw, nn.parallel.DistributedDataParallel): raw = raw.module
    ...
    def on_fit_epoch_end(self, trainer) -> None:
        ...
        if all(v is not None for v in [box_t, cls_t, dfl_t, map50]):
            LOGGER.info(f"[CB-Train] Epoch {epoch:3d} -> box={box_t:.4f} cls={cls_t:.4f} dfl={dfl_t:.4f} mAP@0.5={map50:.4f}")
...
def train(data: str, class_counts: list[int], cfg: Optional[dict] = None, class_names: Optional[list[str]] = None) -> None:
    c = {**NANO_CFG, **(cfg or {})}
    if class_names is None: class_names = read_class_names(data)
    excel_path = resolve_excel_path(c["OUTPUT_XLSX"])

    model = YOLO("yolo26n.pt")
    cbs = CBTrainingCallbacks(output_path=excel_path, class_counts=class_counts, class_names=class_names, beta=c["BETA"])
