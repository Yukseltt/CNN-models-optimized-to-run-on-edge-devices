# train_yolo11n_default.py
# YOLO11-nano training with Ultralytics default hyperparameters and Excel metric logging.
...
def build_excel(output_path: Path, class_names: list[str]) -> None:
    ...
    wb.save(output_path)
    LOGGER.info(f"[v11-Default] Excel created: {output_path}")
...
class DefaultTrainingCallbacks:
    ...
    def on_pretrain_routine_end(self, trainer) -> None:
        ...
        LOGGER.info(f"[v11-Default] Excel will be written to: {self.output_path}")
    ...
    def on_fit_epoch_end(self, trainer) -> None:
        ...
        row = [
            ...
            _fmt(box_t), _fmt(box_v), _fmt(cls_t), _fmt(cls_v),
            _fmt(dfl_t), _fmt(dfl_v), _fmt(prec), _fmt(rec), _fmt(f1),
        ...
        if all(v is not None for v in [box_t, cls_t, dfl_t, map50]):
            LOGGER.info(f"[v11-Default] Epoch {(epoch+1):3d} -> box={box_t:.4f} cls={cls_t:.4f} dfl={dfl_t:.4f} mAP@0.5={map50:.4f}")
        else:
            LOGGER.info(f"[v11-Default] Epoch {(epoch+1)} saved.")
    ...
    def on_train_end(self, trainer) -> None:
        LOGGER.info(f"[v11-Default] Training finished. Output: {self.output_path}")
...
def train(data: str, cfg: Optional[dict] = None, class_names: Optional[list[str]] = None, resume_weight_path: Optional[str] = None) -> None:
    ...
    LOGGER.info(f"[v11-Default] Classes: {class_names}, seed={c['SEED']}")
    LOGGER.info(f"[v11-Default] Runs directory: {c['PROJECT']}")

    cbs = DefaultTrainingCallbacks(output_xlsx_name=Path(c["OUTPUT_XLSX"]).name, class_names=class_names)

    if resume_weight_path:
        LOGGER.info(f"[v11-Default] Egitim devam ettiriliyor: {resume_weight_path}")
        ...
        LOGGER.info("[v11-Default] Yeni egitim baslatiliyor...")
