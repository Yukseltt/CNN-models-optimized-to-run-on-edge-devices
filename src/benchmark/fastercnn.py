...
default_model_path = "/home/simurg/Documents/Python/Yolo/YOLOn8/models/best_faster_rcnn_model.pth"

class FasterRCNNBenchmark:
    def __init__(self, model_path, device="cuda", warmup_frames=30):
        ...
        print(f"Loading Faster R-CNN model: {model_path}")
        # Use weights=None and weights_backbone=None to avoid deprecation warnings and ensure no pretrained weights
        self.model = fasterrcnn_resnet50_fpn(weights=None, weights_backbone=None, num_classes=5)
        checkpoint = torch.load(model_path, map_location=self.device)
        if "model" in checkpoint:
            state = checkpoint["model"]
        else:
            state = checkpoint
        # Load state dict with strict=False to avoid errors due to missing/unexpected keys
        load_result = self.model.load_state_dict(state, strict=False)
        if load_result.missing_keys or load_result.unexpected_keys:
            print("Warning: Issues loading state_dict:")
        ...
        self.class_names = [str(i) for i in range(91)]
    ...
    def warmup_gpu(self, input_size=(640, 640)):
        ...
        for i in range(self.warmup_frames):
            img_tensor = self.preprocess_frame(dummy_image)
            with torch.no_grad():
                _ = self.run_inference(img_tensor)
            if i % 10 == 0:
        ...
        try:
            while True:
                ...
                if display:
                    ...
                    cv2.imshow('Faster R-CNN Benchmark', display_frame)
                frame_count += 1
    ...
    def run_video_benchmark(self, video_path, display=True, save_results=True):
        ...
        try:
            while True:
                ...
                if display:
                    display_frame = self.draw_performance_overlay(annotated_frame)
                    cv2.imshow('Faster R-CNN Video Benchmark', display_frame)
    ...
    def save_benchmark_results(self):
        ...
        filename = f"fasterrcnn_benchmark_{timestamp}.json"
        benchmark_data = {
            "timestamp": timestamp,
...
def main():
    parser = argparse.ArgumentParser(description='Faster R-CNN GPU Benchmark')
    parser.add_argument('--model', type=str, default=default_model_path, help='Path to Faster R-CNN model')
    ...
    print("=== Faster R-CNN GPU Benchmark ===")
    print(f"Model: {args.model}")
    print(f"Device: {args.device}")
    print(f"Source: {args.source}")
    benchmark = FasterRCNNBenchmark(
